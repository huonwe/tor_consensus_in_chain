...
// @@@
use std::thread;
use actix_web::{web, App, get, Responder, HttpResponse, HttpServer};
use tokio;
...
async fn update() -> impl Responder {
	...
	// get tor consensus
	let res = reqwest::get("http://localhost:7000/tor/status-vote/next/authority.z")
		.await.unwrap();
	// println!("Headers:\n{:#?}", res.headers());
    match res.headers().get("content-type") {
        None => {
            return HttpResponse::Ok().body("Get consensus failed. Please try again after a moment.");
        }
        Some(_) => {}
    }
    ...
    rpc_to_localhost("author_submitExtrinsic", [payload_hex])
        .await
        .unwrap();
...
fn main() -> sc_cli::Result<()> {
	let srv = HttpServer::new(|| App::new().service(hello).service(update).default_service(web::to(|| HttpResponse::NotFound())))
        .bind(("localhost",8080))
		.unwrap()
        .run();
	let t1 = thread::spawn(||{
